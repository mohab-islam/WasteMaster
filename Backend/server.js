...
app.post('/api/challenges/join', async (req, res) => {
    try {
        ...
        // Clean up legacy data (strings/IDs) that doesn't match new schema
        // This prevents validation errors on save
        user.joinedChallenges = user.joinedChallenges.filter(jc => jc && jc.challenge);
